...
from utils import accuracy_for_unsupervised_learning
...
elif args.dataset == "CLASSIC3":
    import classic3_specs as specs
elif args.dataset == "CLASSIC4":
    import classic4_specs as specs
elif args.dataset == "BBC":
    import bbc_specs as specs
else:
...
validation = False # args.validation # Specify if data should be split into validation and test sets
...
cpu = True # args.cpu # I choose to only use CPU
if cpu:
    ...
    with tf.Session(config=config) as sess:
        ...
        if pretrain:
            ...
            if validation:
                ...
                acc_cm_permut = accuracy_for_unsupervised_learning(target, kmeans_model.labels_)
                ##print("ACC", acc)
        ...
        for k in range(len(alphas)):
            ...
            if k % print_val == 0 or k == max_n - 1:
                ...
                if validation:
                    ...
                    acc_cm_permut = accuracy_for_unsupervised_learning(target, cluster_assign)
                    ##print("ACC", acc)
        ...
        if validation:
            ...
            list_acc.append(acc)
            list_acc_permut.append(acc_cm_permut)
            list_ari.append(ari)
            list_nmi.append(nmi)
...
if validation:
    ...
    print("ACC: {:.3f} +/- {:.3f}".format(np.max(list_acc), np.std(list_acc)))
    print("Accuracy (CM permutation): {}".format(np.max(list_acc_permut), np.std(list_acc_permut)))
    list_ari = np.array(list_ari)
    print("ARI: {:.3f} +/- {:.3f}".format(np.max(list_ari), np.std(list_ari)))
    list_nmi = np.array(list_nmi)
    print("NMI: {:.3f} +/- {:.3f}".format(np.max(list_nmi), np.std(list_nmi)))
    ##print(list_ari)

    s = "ACC: {:.3f} +/- {:.3f}".format(np.max(list_acc), np.std(list_acc))
    s += "\n"
    s += "Accuracy (CM permutation): {}".format(np.max(list_acc_permut), np.std(list_acc_permut))
    s += "\n"
    s += "ARI: {:.3f} +/- {:.3f}".format(np.max(list_ari), np.std(list_ari))
    s += "\n"
    s += "NMI: {:.3f} +/- {:.3f}".format(np.max(list_nmi), np.std(list_nmi))
    s += "\n"
    with open("dkm_{}.results".format(args.dataset), "w+") as fo:
        fo.write(s)